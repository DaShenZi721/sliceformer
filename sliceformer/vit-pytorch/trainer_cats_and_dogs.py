...
def main(n_it=1, save_adr = 'sinkhorn.npy', save_model = 'sinkhorn.pth', lr=3e-5, epochs = 300, seed=0, wandb=None, attn='trans'):
# Training settings
    batch_size = 64
    gamma = 0.7
    seed = seed
    ...
    for epoch in range(ep, epochs):
        ...
        # attn_weights_array.append(attention_weights.numpy())
        ...
        wandb.log({
            'train loss': epoch_loss.cpu().detach().numpy(),
            'train acc': epoch_accuracy.cpu().detach().numpy(),
            'test loss': epoch_val_loss.cpu().detach().numpy(),
            'test acc': epoch_val_accuracy.cpu().detach().numpy(),
        })
        #scheduler.step()
