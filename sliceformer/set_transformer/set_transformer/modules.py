...
from set_transformer.swd import SWD8
import time
import numpy as np
eps = 1
...
class MABSINK(nn.Module):
    def __init__(self, dim_Q, dim_K, dim_V, num_heads, ln=True, sinkhorn=sinkhornkeops, attn='sink'):
    ...
    def forward(self, Q, K):
        ...
        if self.attn == 'sink':
            ...
            pi = self.sinkhorn(V_)
            O = torch.cat((Q_ + pi).split(Q.size(0), 0), 2)
...
class ISABSINK(nn.Module):
    def __init__(self, dim_in, dim_out, num_heads, num_inds, ln=False, sinkhorn=sinkhornkeops, attn='sink'):
        super(ISABSINK, self).__init__()
        self.I = nn.Parameter(torch.Tensor(1, num_inds, dim_out))
        nn.init.xavier_uniform_(self.I)
        self.mab0 = MABSINK(dim_out, dim_in, dim_out, num_heads, ln=ln, sinkhorn=sinkhorn, attn=attn)
        self.mab1 = MABSINK(dim_in, dim_out, dim_out, num_heads, ln=ln, sinkhorn=sinkhorn, attn=attn)
...
class PMASINK(nn.Module):
    def __init__(self, dim, num_heads, num_seeds, ln=False, sinkhorn=sinkhornkeops, attn='sink'):
        super(PMASINK, self).__init__()
        self.S = nn.Parameter(torch.Tensor(1, num_seeds, dim))
        nn.init.xavier_uniform_(self.S)
        self.mab = MABSINK(dim, dim, dim, num_heads, ln=ln, sinkhorn=sinkhorn, attn=attn)
