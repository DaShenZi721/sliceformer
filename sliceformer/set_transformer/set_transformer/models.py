...
class ModelNetSink(nn.Module):
    def __init__(
        ...
        if attn == 'sink':
            ...
            sinkhornkeops = SWD8()
        self.enc = nn.Sequential(
            ISABSINK(dim_input, dim_hidden, num_heads, num_inds, ln=ln, sinkhorn=sinkhornkeops, attn=attn),
            ISABSINK(dim_hidden, dim_hidden, num_heads, num_inds, ln=ln, sinkhorn=sinkhornkeops, attn=attn),
        )
        self.dec = nn.Sequential(
            nn.Dropout(),
            PMASINK(dim_hidden, num_heads, num_outputs, ln=ln, sinkhorn=sinkhornkeops, attn=attn),
            nn.Dropout(),
            nn.Linear(dim_hidden, dim_output),
        )
