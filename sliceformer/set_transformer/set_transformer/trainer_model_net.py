...
def main(model, n_it, num_pts, learning_rate, batch_size, dim, n_heads, n_anc, train_epochs, save_adr, attn, wandb):
    ...
    if model == 'sinkformer':
        model = ModelNetSink(dim_hidden=dim, num_heads=n_heads, num_inds=n_anc, n_it=n_it, attn=attn)
    else:
