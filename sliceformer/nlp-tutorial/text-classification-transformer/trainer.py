...
class Trainer:
    ...
    def train(self, epoch):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            outputs, attention_weights = self.model(inputs, stage='train')
            ...
            loss.backward()
            self.optimizer.step()
            if attention_weights_cpu is None and attention_weights is not None:
                attention_weights_cpu = attention_weights.cpu()
            # print('finish batch %d in %.4f seconds' % (i, time.time()-start_time))
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.test_loader):
                ...
                outputs, attention_weights = self.model(inputs, stage='valid')
        ...
        losses_b = losses / n_batches
        acc_ns = accs / n_samples * 100.
        print('Valid Epoch: {}\t>\tLoss: {:.4f} / Acc: {:.1f}%'.format(epoch, losses_b, acc_ns))
