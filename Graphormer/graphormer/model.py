...
class Graphormer(pl.LightningModule):
    def __init__(
        ...
        encoders = [EncoderLayer(hidden_dim, ffn_dim, dropout_rate, attention_dropout_rate, num_heads, layer_idx=i)
    ...
    def training_step(self, batched_data, batch_idx):
        if self.dataset_name == 'ogbg-molpcba':
            if not self.flag:
                ...
                optimizer.zero_grad()
                loss, _ = flag_bounded(model_forward, perturb_shape, y_gt[mask], optimizer, batched_data.x.device,
                                       self.loss_fn,
    ...
    def validation_epoch_end(self, outputs):
        ...
        if self.dataset_name == 'ogbg-molpcba':
            ...
            try:
                ...
                wandb.log({
                    'valid_' + self.metric: self.evaluator.eval(input_dict)[self.metric],
                })
            except:
                pass
    ...
    def test_epoch_end(self, outputs):
        ...
        [self.metric], sync_dist=True)
        wandb.log({
            'test_' + self.metric: self.evaluator.eval(input_dict)[self.metric],
        })
...
class EncoderLayer(nn.Module):
    def __init__(self, hidden_size, ffn_size, dropout_rate, attention_dropout_rate, num_heads, layer_idx=None):
        super(EncoderLayer, self).__init__()

        self.self_attention_norm = nn.LayerNorm(hidden_size)
        # self.self_attention = MultiHeadAttention(hidden_size, attention_dropout_rate, num_heads)
        self.self_attention = SWD(hidden_size, attention_dropout_rate, num_heads)
        ...
        if layer_idx is not None:
            self.col_descend = Haar_wavelet_basis(num_col=hidden_size // num_heads, num_basis=2 ** layer_idx)
        else:
            self.col_descend = None
