...
def create_model(flax_module, model_kwargs, key, input_shape):
  ...
  def _create_model(key):
    ...
    with nn.stochastic(key):
      ...
      print("The model size: %d" % param_count)
...
def main(argv):
  ...
  wandb.init(project='text_classfication', mode='disabled')
  initialise_tracking()
  ...
  def run_eval(eval_ds, num_eval_steps=-1):
    ...
    eval_speed = len(eval_metrics)/(time.time()-start_eval_time)
    # logging.info('eval %.4f steps per second'%(len(eval_metrics)/(time.time()-start_eval_time)))
    ...
    return eval_summary, eval_speed
  ...
  best_train_speed, worst_train_speed = 0, float('inf')
  best_eval_speed, worst_eval_speed = 0, float('inf')
  for step, batch in zip(range(start_step, num_train_steps), train_iter):
    ...
    if step % eval_freq == 0 and step > 0:
      if step / eval_freq != 1:
        best_train_speed = max(best_train_speed, eval_freq/(time.time()-start_train_time))
        worst_train_speed = min(worst_train_speed, eval_freq/(time.time()-start_train_time))
        logging.info('train %.4f steps per second'%(eval_freq/(time.time()-start_train_time)))
      ...
      # Eval Metrics
      eval_summary, eval_speed = run_eval(eval_ds, num_eval_steps)
      if step / eval_freq != 1:
        best_eval_speed = max(best_eval_speed, eval_speed)
        worst_eval_speed = min(worst_eval_speed, eval_speed)
        logging.info('eval %.4f steps per second'%(eval_speed))

      best_acc = max(best_acc, eval_summary['accuracy'])
      logging.info('eval in step: %d, loss: %.4f, acc: %.4f, best_acc: %.4f', step,
                   eval_summary['loss'], eval_summary['accuracy'], best_acc)
      if jax.process_index() == 0:
        # Save a Checkpoint
        if config.save_checkpoints and eval_summary['accuracy'] == best_acc:
    ...
    # if step > 510:
    #     break
  print('(train speed) best: %.4f, worst: %.4f\t(eval speed) best: %.4f, worst: %.4f'%(best_train_speed,
                                                                                       worst_train_speed,
                                                                                       best_eval_speed,
                                                                                       worst_eval_speed))
