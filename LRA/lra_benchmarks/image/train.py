...
flags.DEFINE_string('task_name', default='pathfinder32_hard', help='Name of the task')
flags.DEFINE_bool(
...
def test(optimizer, state, p_eval_step, step, test_ds, summary_writer,
  ...
  print('test in step: %d, loss: %.4f, acc: %.4f'%(step,
        test_summary['loss'], test_summary['accuracy']))

  wandb.log({'test_acc':test_summary['accuracy']})
...
def train_loop(config, dropout_rngs, eval_ds, eval_freq, num_eval_steps,
  ...
  best_optimizer = None
  best_state = None
  best_train_acc, best_eval_acc = 0, 0
  step = 0
  for step, batch in zip(range(start_step, num_train_steps), train_iter):
    ...
    if step % eval_freq == 0 and step > 0:
      ...
      best_train_acc = max(best_train_acc, summary['accuracy'])
      # Calculate (clipped) perplexity after averaging log-perplexities:
      logging.info('train in step: %d, loss: %.4f, acc: %.4f, best_acc: %.4f', step,
                   summary['loss'], summary['accuracy'], best_train_acc)
      # print('train in step: %d, loss: %.4f, acc: %.4f, best_acc: %.4f'%( step,
      #       summary['loss'], summary['accuracy'], best_train_acc))
      if jax.process_index() == 0:
        tock = time.time()
      ...
      eval_summary = jax.tree_map(
          ...
          eval_metrics_sums)
      best_eval_acc = max(best_eval_acc, eval_summary['accuracy'])
      logging.info('eval in step: %d, loss: %.4f, acc: %.4f, best_acc: %.4f', step,
                   eval_summary['loss'], eval_summary['accuracy'], best_eval_acc)
      # print('eval in step: %d, loss: %.4f, acc: %.4f, best_acc: %.4f'%( step,
      #       eval_summary['loss'], eval_summary['accuracy'], best_eval_acc))
      if jax.process_index() == 0:
        # Save a Checkpoint
        if config.save_checkpoints and eval_summary['accuracy'] == best_eval_acc:
          # Save unreplicated optimizer + model state.
          checkpoints.save_checkpoint(
              FLAGS.model_dir,
              (jax_utils.unreplicate(optimizer), jax_utils.unreplicate(state)), step)

        if eval_summary['accuracy'] == best_eval_acc:
        ...
        summary_writer.flush()
  return best_optimizer, best_state, step


def main(argv):
  if len(argv) > 1:
    raise app.UsageError('Too many command-line arguments.')
  wandb.init(project='pathfinder32', mode='disabled')
  # initialise_tracking()
