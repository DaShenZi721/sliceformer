...
def main(argv):
  ...
  if FLAGS.test_only:
    ...
    return
  best_train_acc, best_eval_acc, best_test_acc = 0, 0, 0
  metrics_all = []
  tick = time.time()
  ...
  for step, batch in zip(range(start_step, num_train_steps), train_iter):
    ...
    if step % eval_freq == 0 and step > 0:
      ...
      best_test_acc = max(best_test_acc, test_summary['accuracy'])
      logging.info('test in step: %d, loss: %.4f, acc: %.4f, best_acc: %.4f', step,
                   test_summary['loss'], test_summary['accuracy'], best_test_acc)
      if jax.process_index() == 0:
