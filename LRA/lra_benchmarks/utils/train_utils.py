...
from lra_benchmarks.models.sinkformer import sinkformer
from lra_benchmarks.models.sliceformer import sliceformer
import numpy as onp
...
def get_model(model_type, create_model_fn, model_kwargs, *create_model_args):
  ...
  if model_type == 'transformer':
    return create_model_fn(transformer.TransformerEncoder, model_kwargs,
                           *create_model_args)
  elif model_type == 'sliceformer':
    return create_model_fn(sliceformer.SliceformerEncoder, model_kwargs,
                           *create_model_args)
  elif model_type == 'sinkformer':
    return create_model_fn(sinkformer.SinkformerEncoder, model_kwargs,
                           *create_model_args)
  ...
  def step_fn(step):
    ...
    for name in factors:
      if name == 'constant':
        ...
        # progress = jnp.minimum(1.0, (step // warmup_steps))
        # ret *= (progress * 0.9 + 0.1)
